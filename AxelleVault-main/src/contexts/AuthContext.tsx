...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const signUp = async (email: string, password: string, username: string) => {
    ...
    if (error) { isSigningUp.current = false; return { error }; }
    if (!data.user) { isSigningUp.current = false; return { error: { message: 'Signup failed.' } as AuthError }; }

    const { error: insertError } = await supabase.from('user_login_detail').insert({
      id: data.user.id, username, email: normalizedEmail,
      password_hash: null, role: 'free', security_score: 50,
      total_logins: 0, failed_attempts: 0, login_history: [],
      created_at: now, last_login: now,
    });
    if (insertError) console.warn('[Auth] insert warning:', insertError.message);
  ...
  const signIn = async (email: string, password: string) => {
    ...
    if (error || !data.user) {
      const { data: existing } = await supabase.from('user_login_detail')
        .select('login_history, failed_attempts').eq('email', normalizedEmail).maybeSingle();
      if (existing) {
        const history = [...(existing.login_history ?? []), { time: now, success: false }].slice(-10);
        await supabase.from('user_login_detail').update({
          failed_attempts: (existing.failed_attempts ?? 0) + 1, login_history: history,
    ...
    const { data: current } = await supabase.from('user_login_detail')
      .select('total_logins, login_history').eq('id', userId).maybeSingle();
    const history = [...(current?.login_history ?? []), { time: now, success: true }].slice(-10);
    await supabase.from('user_login_detail').update({
      last_login: now, total_logins: (current?.total_logins ?? 0) + 1,
      failed_attempts: 0, login_history: history,
    }).eq('id', userId);
  ...
  const signOut = async () => {
    try {
      setLoading(true);
      if (hasSupabaseEnv) await supabase.auth.signOut({ scope: 'global' });
      clearAllState();
      clearAllStorage();
    } catch (err) {
  ...
  const updateProfile = async (fullName: string) => {
    ...
    const { error } = await supabase.from('user_login_detail')
      .update({ username: fullName, updated_at: new Date().toISOString() }).eq('id', user.id);
